...
- `node tools/test.mjs`: the last line reads `Tests  N passed` with **no
  ...
  **Judge it on failures, not on the count.** At the time of writing it is 45
  ...
  file count *below* 48 means the zip did not fully apply: check the extraction
...
## 10. Check the sign-in page loads (Shell)
...
## 11. Check the copy actually changed (Shell)
...
## 13. Republish. This is the step that delivers the release.
