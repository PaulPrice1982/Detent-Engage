...
**This release fixes the failing health check, and with it the crash loop.**
Two faults, both at start-up. The deployment can no longer die silently, and it
can no longer fail a health check while running perfectly. If something is
still wrong after this, the app stays up and tells you what, in the browser.
...
- `node tools/test.mjs`: the last line reads `Tests  N passed` with **no
  ...
  **Judge it on failures, not on the count.** At the time of writing it is 45
  ...
  file count *below* 46 means the zip did not fully apply: check the extraction
...
Only once step 3 was clean and steps 6, 7, 8 and 9 behaved. Report the output
and **STOP**.
