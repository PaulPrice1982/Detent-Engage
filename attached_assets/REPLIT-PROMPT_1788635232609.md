...
**This release changes what the live site says, which no previous release
managed to do.** The marketing copy lives in the database, and until now the
seed only ran on a site with no pages, so every rewrite since the first deploy
reached nobody. It also fixes the failing health check and the crash loop.
...
## 9. Check the copy actually changed (Shell)
...
Only once step 3 was clean and steps 6 through 10 behaved. Report the output and
**STOP**.
