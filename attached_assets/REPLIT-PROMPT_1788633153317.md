...
**This release fixes the crash loop.** Its whole point is that the deployment
can no longer die silently at start-up. If something is still wrong after this,
the app stays up and tells you what, in the browser, on every page.
...
- `node tools/test.mjs`: the last line reads `Tests  N passed` with **no
  ...
  **Judge it on failures, not on the count.** At the time of writing it is 45
  ...
  file count *below* 45 means the zip did not fully apply: check the extraction
...
Expected: `/health` returns JSON containing `"status":"not_configured"` and a
reason mentioning the database, and the page returns **503**. The process is
still running when you kill it.

A crash, an exit, or `connection refused` is a failure of this release. Report
the exact output and stop.

## 8. Check the app itself (Shell)
...
Only once step 3 was clean and steps 6, 7 and 8 behaved. Report the output and
**STOP**.
...
**The deployment crash looped with the explanation already written and already
on the container.** The page that names a start-up problem existed and was
...
`tests/boot-refusal.test.ts` is the gate that keeps it that way. It starts the
