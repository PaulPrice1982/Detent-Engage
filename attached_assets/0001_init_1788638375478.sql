...
-- No BEGIN or COMMIT in this file. The migration runner wraps each file in one
-- transaction together with the row that records it as applied, so either both
-- land or neither does. A file that commits its own work breaks that: the
-- schema commits, the ledger row does not, and every later start re-runs a
-- migration whose tables already exist. That is not hypothetical. It is what
-- put "relation \"tenant\" already exists" on the live site and kept it there.
--
-- Every statement below is also written to be safe to run again, so a database
-- that already carries this schema is adopted rather than refused.
...
-- A managed database can refuse role creation outright, and that refusal must
-- not take the schema with it. The isolation the application actually depends
-- on is the policy, FORCE ROW LEVEL SECURITY and SET LOCAL app.tenant_id inside
-- every transaction, none of which need this role: it exists so that a second,
-- weaker connection can be handed out later without BYPASSRLS. Where it cannot
-- be created, every grant below is skipped and the isolation is unchanged.
DO $$
BEGIN
...
EXCEPTION WHEN insufficient_privilege THEN
  RAISE NOTICE 'Cannot create the awa_app role here, so its grants are skipped. '
