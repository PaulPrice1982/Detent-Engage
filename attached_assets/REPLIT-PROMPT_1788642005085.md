...
- `"copyRevision":5` and `"consoleHere":true`: the live site has this release.
...
- `node tools/test.mjs`: the last line reads `Tests  N passed` with **no
  ...
  **Judge it on failures, not on the count.** At the time of writing it is 45
  ...
  file count *below* 50 means the zip did not fully apply: check the extraction
...
| `DETENT_CREDENTIAL_KEY` | **New.** 32 bytes of base64: run `openssl rand -base64 32` and paste the result. Encrypts a customer's CRM token at rest. Without it the token is kept in memory rather than written in clear, so every customer reconnects their CRM after each deploy. Once set, do not change it: a rotated key makes stored connections unreadable and every tenant has to reconnect. |
...
`CRM credentials` saying `IN MEMORY` means `DETENT_CREDENTIAL_KEY` is unset and
every customer will reconnect their CRM after each deploy. `payments` saying
`SANDBOX` means the product cannot take money yet. Neither stops the release;
both are things to know.

## 11. Check the sign-in page loads (Shell)
...
Required: `"copyRevision":5`, `"consoleHere":true`, and `live console signin
200`.
