...
/**
 * Shape classes, by aspect ratio. The boundaries are where the rendering
 * decision actually changes, not tidy round numbers.
 */
export type LogoShape =
  /** Square-ish. Fits the launcher beside a label. Up to 1.4:1. */
  | 'mark'
  /** Mark plus a short word. Fits the launcher if the label is dropped. To 3:1. */
  | 'lockup'
  /** A word or phrase. Too wide for the launcher; header and panel only. */
  | 'wordmark'
  /** Taller than wide. Constrained by height and given room, never cropped. */
  | 'tall';
...
export interface TenantLogo {
   ...
   *
   * A data URI is strongly preferred: it needs no `img-src` host in the
   * customer's Content Security Policy, survives an offline first paint, and
   * cannot be swapped by whoever controls the origin it was hosted on.
   */
  readonly source: string;
  readonly format: LogoFormat;
  /** Intrinsic size, used for aspect ratio. Both must be positive. */
  readonly widthPx: number;
  readonly heightPx: number;
  /** Used as the image's alt text. Normally the organisation's name. */
  readonly altText: string;
  /** Optional variant for dark backgrounds. */
  readonly onDark?: string;
  /**
   * Set when the artwork has no transparent margin of its own, so the renderer
   * adds breathing room rather than letting it touch the pill edge.
   */
...
export interface TenantBrand {
  ...
  readonly logo?: TenantLogo;
  /** Launcher background. Any CSS colour; luminance decides the foreground. */
  readonly accentColour?: string;
  /**
   * Whether "Powered by Detent" is hidden in the panel.
   *
   * A commercial entitlement, not a style choice: it is sold with the reseller
   * and enterprise plans. It never hides the AI disclosure, which is a legal
   * obligation and is not ours to sell.
   */
...
export function classifyShape(widthPx: number, heightPx: number): LogoShape {
  if (widthPx <= 0 || heightPx <= 0) return 'mark';
  const ratio = widthPx / heightPx;
  if (ratio < 0.85) return 'tall';
  if (ratio <= 1.4) return 'mark';
  if (ratio <= 3) return 'lockup';
  return 'wordmark';
}
 ...
 */
export function validateLogo(logo: TenantLogo): BrandProblem[] {
  ...
  if (logo.widthPx > 0 && logo.heightPx > 0) {
    const scale = Math.min(
      LAUNCHER_BOX.maxHeightPx / logo.heightPx,
      LAUNCHER_BOX.maxWidthPx / logo.widthPx,
      1,
    );
    if (logo.heightPx * scale < MIN_LEGIBLE_HEIGHT_PX) {
      problems.push({
        field: 'size',
        message:
          'This logo is too wide to read on the launcher, so initials will be shown there instead. ' +
...
function schemeOf(source: string): 'data' | 'https' | 'other' {
...
function dataUriMatchesFormat(source: string, format: LogoFormat): boolean {
 ...
 */
export function luminanceOf(colour: string): number | undefined {
...
function parseColour(colour: string): [number, number, number] | undefined {
...
export type LogoTreatment =
  /** Draw the logo directly on the background. */
  | 'direct'
  /** Draw the supplied dark-background variant. */
  | 'variant'
  /** Place a light chip behind the logo so it reads on any background. */
  | 'chip';
...
export interface LogoPlan {
  ...
  /** The source to render, after variant selection. */
  readonly source: string;
  readonly widthPx: number;
  readonly heightPx: number;
  /** Whether the launcher's text label should be hidden to make room. */
  readonly hideLabel: boolean;
  /**
   * True when the logo cannot be rendered legibly in this box and a monogram
   * should be drawn instead. The logo itself is still used where there is room.
   */
  readonly useMonogram: boolean;
  readonly altText: string;
}
...
export const LAUNCHER_BOX: PlacementBox = { maxHeightPx: 18, maxWidthPx: 92 };
/** The panel header: room for a full wordmark. */
export const PANEL_HEADER_BOX: PlacementBox = { maxHeightPx: 24, maxWidthPx: 168 };
...
export function planLogo(
  logo: TenantLogo,
  box: PlacementBox,
  options: { readonly backgroundColour?: string; readonly isLauncher?: boolean } = {},
): LogoPlan {
  ...
  if (logo.format === 'jpeg' || logo.needsPadding) treatment = 'chip';
...
export function monogramOf(organisationName: string): string {
